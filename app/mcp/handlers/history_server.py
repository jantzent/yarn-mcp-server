from typing import Any, Dict

from app.clients.history_server import HistoryServerClient
from app.models import AppIdParams, AppListParams, AttemptIdParams


class HistoryServerHandler:
    def __init__(self, client: HistoryServerClient) -> None:
        self._client = client

    async def apps(self, params: AppListParams) -> Dict[str, Any]:
        return await self._client.apps(params=params.model_dump(exclude_none=True))

    async def app_details(self, params: AppIdParams) -> Dict[str, Any]:
        return await self._client.app(params.app_id)

    async def app_attempts(self, params: AppIdParams) -> Dict[str, Any]:
        return await self._client.app_attempts(params.app_id)

    async def app_containers(self, params: AttemptIdParams) -> Dict[str, Any]:
        return await self._client.app_containers(params.app_id, params.attempt_id)
